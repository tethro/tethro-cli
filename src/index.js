 ...
 *   tethro panic [session-id]                        Kill + restore latest healthy checkpoint
 *   tethro init                                      First-run: token + console URL + doctor
...
function getIsolationMethod(requested) {
  ...
  if (requested === "firecracker" && commandExists("firecracker")) return "firecracker";
  if (requested === "gvisor" && commandExists("runsc")) return "gvisor";
  if (requested === "docker" && commandExists("docker")) return "docker";
...
async function runInSubprocess(sandboxDir, agent, prompt, sessionId) {
  ...
  return { child, method: "subprocess", pid: child.pid };
}
...
async function cmdRun(agent, prompt, opts = {}) {
  ...
  // Healthy checkpoint before the agent touches the tree (Indie rollback wedge).
  const ckpt = createLocalCheckpoint(sandboxDir, sessionId);
  log(`Checkpoint: ${ckpt.files} files → ${ckpt.path}`, "ok");

  // Run agent in sandbox
  const { child, method, pid } = await runInSandbox(sandboxDir, agent, prompt, isolation, sessionId);
  const agentPid = pid || child.pid;
  registerLocalSession(sessionId, {
    pid: agentPid,
    pgid: process.platform !== "win32" ? agentPid : null,
    startKey: null,
    workspace: sandboxDir,
  });

  // Tell the console this process lives on the CLI machine (kill/restore via relay).
  await apiRequest("PATCH", `/api/sessions/${sessionId}`, {
    agentHost: "cli",
    agentPid,
    agentPgid: process.platform !== "win32" ? agentPid : null,
    workspacePath: sandboxDir,
    status: "running",
  }).catch(() => {
    log("Could not register CLI agentHost with console (relay kill may not work)", "warn");
  });

  log(`Agent running with ${method} isolation (pid ${agentPid})`, "ok");
  ...
  child.on("close", (code) => {
    ...
    unregisterLocalSession(sessionId);
    ...
    // Keep sandbox on failure so relayed restore / panic can still run.
    if (process.env.TETHRO_KEEP_SANDBOX !== "1" && code === 0) {
      try {
        fs.rmSync(sandboxDir, { recursive: true, force: true });
        log(`Sandbox cleaned up`, "ok");
      } catch {}
    } else if (code !== 0) {
      log(`Sandbox kept at ${sandboxDir} (failed exit — panic/restore available)`, "warn");
    }
...
async function cmdPanic(sessionId) {
  log("Panic — kill + rollback to latest healthy checkpoint...", "warn");
  try {
    const res = await apiRequest("POST", "/api/panic", sessionId ? { sessionId } : {});
    if (res.status === 401) {
      log("Unauthorized — set TETHRO_TOKEN", "error");
      process.exit(1);
    }
    if (res.data?.upgradeRequired) {
      log("Agent killed, but rollback requires Indie Solo ($7/mo)", "warn");
      log("Upgrade for auto-rollback-on-failure + panic restore", "info");
    }
    const results = res.data?.results || [];
    for (const r of results) {
      const rb = r.rollback || {};
      log(
        `${r.sessionId}: killed=${r.killed} rollback=${rb.ok ? "ok" : "skipped"} ${rb.detail || ""}`,
        rb.ok ? "ok" : "warn"
      );
    }
    if (!results.length) {
      log(res.data?.error || "No session to panic", "warn");
    }
  } catch (e) {
    log(`Panic failed: ${e.message}`, "error");
    process.exit(1);
  }
}
...
async function cmdDaemon(action) {
  if (action === "start") {
    if (!API_TOKEN) {
      log("TETHRO_TOKEN required for kill/restore relay — run tethro init", "error");
      process.exit(1);
    }
    log("Starting Tethro daemon (kill/restore relay sidecar)...", "info");
    log(`Console: ${CONSOLE_URL}`, "info");
    log("Polling /api/relay/poll every 1s for kill:<session> / restore:<session>", "info");
    ...
    const poll = async () => {
      try {
        const res = await apiRequest("GET", "/api/relay/poll");
        if (res.status === 401) {
          log("Unauthorized — check TETHRO_TOKEN", "error");
          return;
        }
        if (res.data?.configured === false) {
          // Console missing Upstash — warn once per minute via throttle
          if (!poll._warned) {
            log(res.data?.detail || "Relay not configured on console", "warn");
            poll._warned = true;
          }
          return;
        }
        ...
        if (cmd) {
          log(`Relay command: ${cmd.op} ${cmd.sessionId}`, "warn");
          await handleRelayCommand(cmd, apiRequest, log);
        }
      } catch (e) {
        log(`Relay poll error: ${e.message}`, "warn");
      }
    };
    ...
    try {
      ...
      log(`Local relay sessions: ${local.length}`, "ok");
      const relay = await apiRequest("GET", "/api/relay/poll");
      log(
        `Relay: ${relay.data?.configured ? "configured" : "not configured on console"}`,
        relay.data?.configured ? "ok" : "warn"
      );
    } catch {
...
async function cmdInit() {
  log("Tethro init — first-run setup", "info");
  const configDir = path.join(os.homedir(), ".tethro");
  fs.mkdirSync(configDir, { recursive: true });
  const envPath = path.join(configDir, "env");

  console.log("");
  console.log("1. Sign in to the console and create a CLI token.");
  console.log(`2. Console URL (default ${CONSOLE_URL})`);
  console.log("3. Export for this shell, or write ~/.tethro/env:");
  console.log("");
  console.log(`   export TETHRO_URL=${CONSOLE_URL}`);
  console.log("   export TETHRO_TOKEN=tethro_...");
  console.log("");
  console.log("4. In another terminal: tethro daemon start");
  console.log('5. Then: tethro run claude-code "your prompt"');
  console.log("");

  const lines = [
    `# Tethro CLI — generated by tethro init`,
    `TETHRO_URL=${CONSOLE_URL}`,
    `TETHRO_TOKEN=${API_TOKEN || "tethro_REPLACE_ME"}`,
    `TETHRO_ISOLATION=${DEFAULT_ISOLATION}`,
    "",
  ];
  fs.writeFileSync(envPath, lines.join("\n"));
  log(`Wrote ${envPath}`, "ok");
  log("Source it:  set -a; source ~/.tethro/env; set +a   (or setx on Windows)", "info");

  try {
    const res = await apiRequest("GET", "/api/health");
    if (res.status === 200) {
      log(`Console health OK at ${CONSOLE_URL}`, "ok");
    } else {
      log(`Console returned ${res.status} — check TETHRO_URL`, "warn");
    }
  } catch {
    log(`Console not reachable at ${CONSOLE_URL}`, "warn");
  }

  log("Next: tethro doctor && tethro daemon start", "info");
}
...
async function cmdDoctor() {
  ...
  if (commandExists("npx")) {
    log("npx: available (claude-code, codex installable)", "ok");
  } else {
...
COMMANDS
  ...
  panic [session-id]        Kill agent + restore latest healthy checkpoint (Indie+)
  init                      First-run setup (writes ~/.tethro/env)
